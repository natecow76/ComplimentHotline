...
# Database filename
DB_FILENAME = 'complimentbot_database.db'
...
def get_user(user_id):
    ...
    try:
        ...
        cursor.execute('SELECT free_interactions_used, credits FROM users WHERE user_id = ?', (user_id,))
        result = cursor.fetchone()
        
        if result:
            user_data = {'free_interactions_used': result[0], 'credits': result[1]}
            logger.debug(f"Retrieved existing user {user_id}: {user_data}")
        else:
            # If user doesn't exist, create a new record with 0 credits
            cursor.execute('INSERT INTO users (user_id, credits) VALUES (?, ?)', (user_id, 0))
            conn.commit()
            user_data = {'free_interactions_used': 0, 'credits': 0}
            logger.debug(f"New user {user_id} created with 0 credits.")
...
def update_user(user_id, free_interactions_used=None, credits=None):
    ...
    try:
        ...
        if fields:
            ...
            conn.commit()
            logger.debug(f"Updated user {user_id}: free_interactions_used={free_interactions_used}, credits={credits}")
...
def add_credits(user_id, credits_to_add):
    ...
    try:
        ...
        logger.debug(f"Added {credits_to_add} credits to user {user_id}. New balance: {new_credits}")
    except Exception as e:
...
def consume_credit(user_id):
    ...
    try:
        ...
        if user['credits'] >= 1:
            ...
            update_user(user_id, credits=new_credits)
            logger.debug(f"Consumed 1 credit from user {user_id}. Remaining credits: {new_credits}")
            return True
        else:
            logger.debug(f"User {user_id} has no credits to consume.")
            return False
    except Exception as e:
